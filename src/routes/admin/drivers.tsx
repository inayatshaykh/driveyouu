...
function DriversPage() {
  ...
  const filtered = useMemo(() => {
    return DRIVERS.filter(d => {
      ...
      const matchKyc = kycFilter === 'all' || d.kyc === kycFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage driver accounts, KYC and performance</p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {(['all', 'active', 'offline', 'suspended'] as const).map(s => (
          ...
          >
            {s} ({counts[s as keyof typeof counts] ?? DRIVERS.length})
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.length === 0 ? (
          ...
          filtered.map(driver => {
            ...
            const kycCfg = KYC_CONFIG[driver.kyc];
            const KycIcon = kycCfg.icon;
            return (
              <div key={driver.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition-all duration-200 hover:shadow-lg hover:shadow-slate-900/50">
                {/* Card Header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="h-12 w-12 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                        {driver.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div className={`absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 ${statusCfg.dot}`} />
                    </div>
                    <div>
                  ...
                  <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${statusCfg.color}`}>
                    {statusCfg.label}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                  <div className={`flex items-center gap-1.5 text-xs font-medium ${kycCfg.color}`}>
                    <KycIcon className="h-3.5 w-3.5" />
                    KYC {kycCfg.label}
                  </div>
                  <div className="text-xs text-slate-500">Joined {driver.joined}</div>
                </div>
              </div>
            );
          })
